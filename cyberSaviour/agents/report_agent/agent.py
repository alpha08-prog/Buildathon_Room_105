...
class ReportAgent(Body):
    ...
    def process(self, state: dict) -> dict:
        ...
        try:
            ...
            # Rule-based fallback: build a structured report from pipeline context
            ctx = state.get('context', {})
            decision  = ctx.get('decision', {})
            action    = ctx.get('action_result', {})
            threat    = ctx.get('threat_intel', {})
            mitre     = threat.get('mitre_anchor', {})
            ips       = action.get('ips_acted_on', [])
            report_text = (
                f"INCIDENT REPORT — {timestamp}\n"
                f"{'='*50}\n"
                f"Priority   : {decision.get('priority', 'unknown').upper()}\n"
                f"Action     : {decision.get('action', 'unknown').upper()}\n"
                f"IPs Acted  : {', '.join(ips) if ips else 'none'}\n"
                f"MITRE      : {mitre.get('id','?')} — {mitre.get('technique','?')} ({mitre.get('tactic','?')})\n"
